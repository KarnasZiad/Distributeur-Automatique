...
public class TransactionService {
    ...
    public Transaction selectProduct(Long productId) {
        ...
        if (!product.isAvailable()) {
            throw new IllegalStateException("Produit non disponible");
        }
        
        if (!transaction.canPurchase(product)) {
            throw new IllegalStateException("Solde insuffisant. Solde actuel: " + 
                transaction.getTotalInserted() + " MAD, Prix: " + product.getPrice() + " MAD");
        }
        
        BigDecimal changeAmount = transaction.getChangeAmount();
        if (!changeService.canMakeChange(changeAmount)) {
            throw new IllegalStateException("Impossible de rendre la monnaie exacte");
        }
